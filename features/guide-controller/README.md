...
The extension implementation lives in `apps/extension/guide/guide-controller.ts`. It resolves live registry targets when an action supplies one, scrolls with reduced-motion support, remeasures after layout changes, keeps the focus mask aligned with the live target, waits for transition-aligned companion movement to settle before speech, speaks validated consequence explanations before consequential manual-action instructions, speaks targetless general explanations without inventing a target, drives injected overlay/companion/speech adapters, preserves pending context for explanations, and exposes cancellation without any automatic click capability.
...
10. play the consequence explanation and spoken instruction when required
