...
test('Groq adapter sends a direct JSON reasoning request', async () => {
  ...
  assert.match(payload.messages[0].content, /Page-entry requirements are values/);
  assert.match(payload.messages[0].content, /Never ask the user to speak/);
...
test('Groq adapter guides the first page-entry textbox when the model misclassifies fields as missing information', async () => {
  const loginElements = [
    {
      id: 'el_uan',
      role: 'textbox',
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  action: 'clarify',
                  targetId: null,
                  spokenInstruction: 'Please provide your UAN and password.',
                  consequence: null,
                  expectedUserAction: null,
                  language: 'en-IN',
                  workflow: {
                    intent: 'PF withdrawal',
                    requiredInformation: ['UAN', 'Password'],
                    knownInformation: [],
                    missingInformation: ['UAN', 'Password'],
                    readiness: 'needs_clarification',
                    clarifyingQuestion: 'Please provide your UAN and password.',
                  },
                }),
              },
            },
  ...
  const result = await reasoner.reason(
    validateReasonRequest({
      userUtterance: 'I want to withdraw money from my PF.',
      userLanguage: 'en-IN',
      session: { goal: 'PF withdrawal', recentActions: [] },
      page: {
        title: 'Unified Member Portal',
  ...
  assert.deepEqual(result, {
    ...
    targetId: 'el_uan',
    spokenInstruction:
      'Click Universal Account Number (UAN). Enter it on the website yourself. Do not say it aloud. Say "I\'m done" when finished.',
...
test('Groq adapter recovers a where-to-enter question into page-entry guidance', async () => {
  const reasoner = createGroqReasoner({
    baseUrl: 'https://api.groq.test/openai/v1',
    model: 'openai/gpt-oss-120b',
    apiKey: 'test-groq-key',
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  action: 'explain',
                  targetId: null,
                  spokenInstruction: 'The login form needs your UAN and password.',
                  consequence: null,
                  expectedUserAction: null,
                  language: 'en-IN',
                  workflow: {
  ...
  const result = await reasoner.reason(
    validateReasonRequest({
      userUtterance: 'Where should I enter it?',
      userLanguage: 'en-IN',
      session: { goal: 'PF withdrawal', recentActions: [] },
      page: {
        title: 'Unified Member Portal',
  ...
  assert.equal(result.action, 'guide');
  assert.equal(result.targetId, 'el_uan');
  assert.equal(result.expectedUserAction, 'input');
  assert.match(result.spokenInstruction, /I'm done/);
});
