...
The companion control is wired to the extension runtime and exposes accessible state, retry, follow-up, demo-reset, and slow-work semantics. The DOM semantic layer scans accessible interactive candidates, infers bounded labels and section context, tracks visibility separately from viewport position, and exposes a debounced `MutationObserver` scanner. Session state, action observation, reasoning, guidance, voice, safety, flow orchestration, runtime composition, accessibility, demo reset, and loading latency are covered by their respective feature boundaries.
